...
`;
  ...
  if (bmadRoot) {
    const templatePaths = [
      path.join(bmadRoot, 'src', 'modules', moduleName, 'config-template.yaml'),
      path.join(bmadRoot, 'src', moduleName, 'config-template.yaml'),
      path.join(bmadRoot, 'src', 'core', 'config-template.yaml'), // For core module
    ];
    ...
    for (const templatePath of templatePaths) {
      if (await fs.pathExists(templatePath)) {
        try {
          ...
          console.log(`  📄 Using BMAD template for ${moduleName}`);
  ...
  const defaultContent =
    DEFAULT_CONFIG_TEMPLATES[moduleName] ||
    '# Add module-specific configuration below\n';
  return header + defaultContent;
}
...
/**
 * Detect BMAD version and validate expected structure
...
async function main() {
  ...
  try {
    ...
    // Step 1.5: BMAD version detection
    await detectBmadVersion(bmadRoot);
    ...
    // Step 6: Migrate auxiliary resources (from config)
