...
During BMAD INSTALLATION, sidecar folder is copied from the agent location to `{runtime-memory}/{sidecar-folder}/`

**ALL agent YAML references MUST use:**

```yaml
{runtime-memory}/{sidecar-folder}/{file}
```
...
# ✅ CORRECT
critical_actions:
  - "Load COMPLETE file {runtime-memory}/journal-keeper-sidecar/memories.md"
  - "ONLY read/write files in {runtime-memory}/journal-keeper-sidecar/"

menu:
  - action: "Update {runtime-memory}/journal-keeper-sidecar/memories.md with insights"
```
...
agent:
  ...
  critical_actions:
    - 'Load COMPLETE file {runtime-memory}/{sidecar-folder}/memories.md'
    - 'Load COMPLETE file {runtime-memory}/{sidecar-folder}/instructions.md'
    - 'ONLY read/write files in {runtime-memory}/{sidecar-folder}/'
  ...
  menu:
    ...
    - trigger: SM or fuzzy match on save
      action: 'Update {runtime-memory}/{sidecar-folder}/memories.md with insights'
...
# Read-only knowledge
- 'Load from {runtime-memory}/{sidecar-folder}/knowledge/ but NEVER modify'
- 'Write ONLY to {runtime-memory}/{sidecar-folder}/memories.md'
...
- Proper `{runtime-memory}/` path format
...
- [ ] **ALL paths use: `{runtime-memory}/{sidecar-folder}/...`**
