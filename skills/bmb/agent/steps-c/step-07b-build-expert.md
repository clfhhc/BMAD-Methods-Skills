...
# Task References
advancedElicitationTask: '../../core/advanced-elicitation/SKILL.md'
partyModeWorkflow: '../../core/party-mode/SKILL.md'
---
...
2. **CRITICAL_ACTIONS FORMAT**: All critical_actions MUST use `{runtime-memory}/{sidecar-folder}/` for file operations (runtime path)
...
critical-actions:
  - name: '{action-name}'
    ...
    sidecar-files:
      - '{runtime-memory}/{sidecar-folder}/{file1}.md'
      - '{runtime-memory}/{sidecar-folder}/{file2}.md'
...
metadata:
  sidecar-folder: '{sidecar-folder-name}'
  sidecar-path: '{runtime-memory}/{sidecar-folder}/'
  agent-type: 'expert'
...
3. **Add README to Sidecar**:
   ...
   `{runtime-memory}/{sidecar-folder}/{filename}.md`
   ```
...
✅ critical_actions reference `{runtime-memory}/{sidecar-folder}/` paths
